...
module "disk_free_percent" {
  ...
  query            = "avg(${var.disk_free_percent_evaluation_period}):( min:system.disk.free{${local.disk_free_percent_filter}} by {host,device} / min:system.disk.total{${local.disk_free_percent_filter}} by {host,device} ) * 100 < ${var.disk_free_percent_critical}"
